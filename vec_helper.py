...
def vecs2coords(vectors, space_size):
	# pivot at node 0
	coords = np.zeros((len(vectors), 2))
	for i in range(1, len(vectors)):
		if not is_missing(vectors[0][i]):
			coords[i] = vectors[0][i]
		else:
			for j in range(len(vectors)):
				if j != i and j != 0 and not is_missing(vectors[0][j]) and not is_missing(vectors[j][i]):
					coords[i] = vectors[0][j] + vectors[j][i]
					break
	gm = np.min(coords)
	if gm < 0:
		coords = coords - gm
